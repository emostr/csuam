...
write_env() {
	...
	local db_root db_pass minio_pass jwt seed_head seed_lib seed_teacher
	db_root=$(resolve_secret MARIADB_ROOT_PASSWORD)
	db_pass=$(resolve_secret MARIADB_PASSWORD)
	minio_pass=$(resolve_secret MINIO_ROOT_PASSWORD)
	jwt=$(resolve_secret JWT_SECRET)
	seed_head=$(resolve_secret SEED_HEAD_TEACHER_PASSWORD)
	seed_lib=$(resolve_secret SEED_LIBRARIAN_PASSWORD)
	seed_teacher=$(resolve_secret SEED_TEACHER_PASSWORD)
...
SEED_HEAD_TEACHER_PASSWORD=$seed_head
SEED_LIBRARIAN_PASSWORD=$seed_lib
SEED_TEACHER_PASSWORD=$seed_teacher
EOF
	chmod 600 "$ENV_FILE"
	ok "записан $ENV_FILE"
}
...
backup_database() {
	...
	local container
	container=$(cd "$APP_DIR" && docker compose ps -q mariadb 2>/dev/null || true)
	...
	pass=$(env_value MARIADB_ROOT_PASSWORD || true)
	if docker exec -e MYSQL_PWD="$pass" "$container" \
		mariadb-dump -uroot --databases "$MARIADB_DATABASE" 2>/dev/null | gzip >"$dump"; then
...
wait_for_app() {
	...
	die "приложение не поднялось за 2 минуты — смотрите: cd $APP_DIR && docker compose logs"
}

ensure_caddy_import() {
	mkdir -p "$CADDY_SITE_DIR"
	if [[ ! -f $CADDY_MAIN ]]; then
		printf '# Управляется csuam deploy.sh\n%s\n' "$CADDY_IMPORT_LINE" >"$CADDY_MAIN"
		return
	fi
...
write_caddy_site() {
	...
	{
		printf '# Управляется csuam deploy.sh — правки перезапишутся при следующем запуске.\n'
		printf '%s {\n' "$site_address"
...
summary() {
	local seed_head
	seed_head=$(env_value SEED_HEAD_TEACHER_PASSWORD || true)
	local scheme=https
	[[ $TLS_MODE == off ]] && scheme=http
	...
	printf '  Вход завуча:      zavuch / %s\n' "${seed_head:-см. .env}"
	printf '  Пароли учителя и библиотекаря — в %s\n\n' "$ENV_FILE"
